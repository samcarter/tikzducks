...
In case the duck should not completely random, but only some of the random elements are desired, these can be acceded independently by using \lstinline|\randomhead| and \lstinline|\randomaccessories| for a random headpiece and a random accessories, respectively. Small caveat: when using \lstinline|\randomhead| or \lstinline|\randomaccessories| more then once, \lstinline|\shuffleducks| has to be used in between (for \lstinline|\randuck| this is done automatically).     
